      ...
      try {
        ...
        const norm: IgItem[] = data.map((raw: any) => {
          if (typeof raw === "string") {
            const isIg = /https?:\/\/(www\.)?instagram\.com\//i.test(raw)
            return {
              img: isIg ? `${import.meta.env.BASE_URL}ig/placeholder.svg` : raw,
              href: raw,
              alt: "Instagram"
            }
          }
          const href = typeof raw.href === "string" ? raw.href : undefined
          let img = typeof raw.img === "string" ? raw.img : ""
          const isIg = /https?:\/\/(www\.)?instagram\.com\//i.test(img)
          if (isIg) img = `${import.meta.env.BASE_URL}ig/placeholder.svg`
          if (!/^https?:\/\//i.test(img)) {
            ...
            const card = (
              <div className="group block rounded-2xl overflow-hidden border border-slate-200 bg-slate-100">
                {img ? (
                  ...
                  <div className="w-full h-48 flex items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 text-slate-600 text-sm">Ver en Instagram</div>
                )}
              </div>
            )
            return it.href ? (
...
function ErrorBoundary({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    const onError = (ev: ErrorEvent) => setErr(ev.error || new Error(ev.message))
    window.addEventListener('error', onError)
    return () => window.removeEventListener('error', onError)
  }, [])
  if (err) {
    return (<div className="p-6 max-w-[1100px] mx-auto">
      <h2 className="text-lg font-semibold text-rose-600">Algo ha fallado</h2>
      <p className="text-slate-700 mt-2">Revisa la consola del navegador (F12) para más detalles.</p>
      <pre className="mt-3 text-xs bg-slate-100 p-3 rounded">{String(err?.stack || err?.message)}</pre>
    </div>)
  }
  return <>{children}</>
}
